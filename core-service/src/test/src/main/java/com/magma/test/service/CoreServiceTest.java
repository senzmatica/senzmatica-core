...
import com.magma.service.util.Granularity;
...
public class CoreServiceTest {
    ...
    public static void getGraphDataOfKitByNumber(String from, String to, Granularity granularity) {
        ...
        for (int i = 0; i < Integer.MAX_VALUE; i++) {
            ...
            DateTime fromDateTime = granularity == Granularity.DAY ? fromDateTimeStart.plusDays(i)
                    : fromDateTimeStart.plusHours(i);
            DateTime toDateTime = granularity == Granularity.DAY ? fromDateTimeStart.plusDays(i + 1)
                    : fromDateTimeStart.plusHours(i + 1);

            DateTime toDateTimeEndTime = granularity == Granularity.DAY ? toDateTimeEnd.plusDays(1)
                    : toDateTimeEnd.plusHours(1);
