...
public class ProductService {
     ...
     */
    public void doHandleProductVersionUpdates(String deviceId, String message) {
        logger.debug("Device -{} To System Message to Update the product version of device-{}", deviceId, message);

        // Validate DeviceId
        Device currentDevice = deviceRepository.findById(deviceId).orElse(null);
        ...
        if (message == null || message.isEmpty()) {
            logger.debug("Empty message received from the device. Ignoring.");
            return; // No need to proceed further if the message is empty
        }

        String[] messageElements = message.split("\\|\\|");

        if (messageElements.length < 2 || messageElements[1].length() < 5) {
            logger.debug("Invalid message received from the device.");
            throw new MagmaException(MagmaStatus.INVALID_INPUT);
        }

        String currentVersionInMessage = messageElements[3].substring(4);


        if (!currentVersionInMessage.equals(currentDevice.getProduct().getCurrentProductVersion())) {
            logger.debug("Invalid message received from the device.");
            throw new MagmaException(MagmaStatus.INVALID_INPUT);
        }
        ...
        switch (messageElements[0]) {
            case "0-get-o:2":
                logger.debug("New current version not available for device");
                currentDevice.getProduct().setCurrentVersionStatus(TestResult.FAILURE.toString());
                changeTestResultsOfDeviceInsideVersion(currentDevice.getProduct().getProductId(), currentDevice.getProduct().getCurrentProductVersion(), deviceId, TestResult.FAILURE);
                // Remove the product from ProductRepository if it exists
                if (product != null) {
            ...
            case "0-get-o:0":
                logger.debug("New current version OTA download Failure");
                currentDevice.getProduct().setCurrentVersionStatus(TestResult.FAILURE.toString());
                changeTestResultsOfDeviceInsideVersion(currentDevice.getProduct().getProductId(), currentDevice.getProduct().getCurrentProductVersion(), deviceId, TestResult.FAILURE);
                // Remove the product from ProductRepository if it exists
                if (product != null) {
            ...
            case "0-get-o:1":
                ...
                currentDevice.getProduct().setCurrentVersionStatus(TestResult.SUCCESS.toString());
                changeTestResultsOfDeviceInsideVersion(currentDevice.getProduct().getProductId(), currentDevice.getProduct().getCurrentProductVersion(), deviceId, TestResult.SUCCESS);
                currentDevice.setProduct(product);
                DeviceParameterConfiguration currentConfiguration = deviceParameterConfigurationRepository.findByDevice(deviceId);
                ...
                List<ProductVersion> filteredVersions = versions.stream()
                        .filter(version -> version.getVersionNum().equals(product.getCurrentProductVersion()))
                        .collect(Collectors.toList());
                currentConfiguration.setRemoteConfigurations(filteredVersions.get(0).getRemoteConfigurations());
                currentConfiguration.setUpdateHistory(new ArrayList<>());
                currentDevice.setDeviceParameterConfiguration(currentConfiguration);
                deviceParameterConfigurationRepository.save(currentConfiguration);
...
//            MagmaCodec magmaCodec = magmaCodecRepository.findById(productTypeDTO.getCodecName());
//            if(magmaCodec==null){
