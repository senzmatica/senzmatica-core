    ...
    public CorporateSensorSummary getCorporateWiseSensorSummary(String corporateId) {
        ...
        for (Kit kit : KitsInCorporate) {
            ...
            for (String psCode : allPropertyCodesOfKit) {
                SensorCode sensorCode = sensorCodeRepository.findByCode(psCode);
                if (!sensorCodesInCorporate.contains(psCode)) {
                    sensorCodesInCorporate.add(psCode);
                    sensorNamesInCorporate.add(sensorCode.getCodeValue());
                }
                propertySensorSummaryMap.putIfAbsent(sensorCode.getCodeValue(), new HashMap<>());
                propertySensorSummaryDeviceMap.putIfAbsent(sensorCode.getCodeValue(), new HashMap<>()); //To store devices list
                Map<String, Integer> propertySensorSpecificMap = propertySensorSummaryMap.get(sensorCode.getCodeValue());
                Map<String, List<Device>> propertySensorSpecificDeviceMap = propertySensorSummaryDeviceMap.get(sensorCode.getCodeValue());
            ...
            for (String npsCode : allNonPropertyCodesOfKit) {
                SensorCode sensorCode = sensorCodeRepository.findByCode(npsCode);
                if (!sensorCodesInCorporate.contains(npsCode)) {
                    sensorCodesInCorporate.add(npsCode);
                    sensorNamesInCorporate.add(sensorCode.getCodeValue());
                }
                nonPropertySensorSummaryMap.putIfAbsent(sensorCode.getCodeValue(), new HashMap<>());
            }
            ...
            for (Property property : kit.getProperties()) {
                if (allPropertyCodesOfKit.contains(property.getCode())) {
                    ...
                    SensorCode sensorCode = sensorCodeRepository.findByCode(property.getCode());
                    Map<String, Integer> propertySensorSpecificMap = propertySensorSummaryMap.get(sensorCode.getCodeValue());
            ...
            for (String psCode : allPropertyCodesOfKit) {
                SensorCode sensorCode = sensorCodeRepository.findByCode(psCode);
                Map<String, List<Device>> propertySensorSpecificDevicesMap = propertySensorSummaryDeviceMap.get(sensorCode.getCodeValue());
            ...
            for (Device device : allDevicesInKit) {
                ...
                for (Sensor nonPropertySensor : nonPropertySensorReadingsOfDevice) {
                    SensorCode sensorCode = sensorCodeRepository.findByCode(nonPropertySensor.getCode());
                    Map<String, Integer> nonPropertySensorSpecificMap = nonPropertySensorSummaryMap.get(sensorCode.getCodeValue());
